...
def get_bn(data, n_bins=5, has_mixture=True):
    """
    ...
    Args:
        data (pd.DataFrame): Input dataset containing continuous and/or discrete features.
        n_bins (int, optional): Number of bins to use for discretizing continuous features. 
            Defaults to 10.
        has_mixture (bool, optional): Whether to use mixture models for continuous variables in 
            HybridBN. Ignored if the BN is discrete. Defaults to True.

    Returns:
        HybridBN | DiscreteBN: A configured Bayesian Network. HybridBN is returned if continuous 
            features are detected, otherwise DiscreteBN is used.
